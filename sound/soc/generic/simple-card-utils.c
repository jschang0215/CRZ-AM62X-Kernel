...
// From wm8960_hw_params
	{
		...
		for_each_rtd_cpu_dais(rtd, i, sdai) {
        	ret = snd_soc_dai_set_sysclk(sdai, DAVINCI_CLK_AHCLKX, sysclk, SND_SOC_CLOCK_OUT);
        	ret = snd_soc_dai_set_sysclk(sdai, DAVINCI_CLK_AUX, sysclk, SND_SOC_CLOCK_OUT);
        	//ret = snd_soc_dai_set_clkdiv(sdai, DAVINCI_CLK_AUX, 17);
        	ret = snd_soc_dai_set_clkdiv(sdai, DAVINCI_CLK_AUX, 16);
        }
	}
	return 0;
}
