...
static void mcasp_start_rx(struct davinci_mcasp *mcasp)
{
	 ...
	 */
        MANGO_DBG("mcasp_is_synchronous(mcasp)=%d\n",mcasp_is_synchronous(mcasp));
					 ...
					 unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_BP_FP:
		MANGO_DBG("SND_SOC_DAIFMT_CBS_CFS codec is clock and frame slave\n");
	...
	case SND_SOC_DAIFMT_BP_FC:
		MANGO_DBG("SND_SOC_DAIFMT_CBS_CFM codec is clock slave and frame master \n");
	...
	case SND_SOC_DAIFMT_BC_FP:
               MANGO_DBG("SND_SOC_DAIFMT_CBM_CFS  codec is clock master and frame slave\n");
	...
	case SND_SOC_DAIFMT_BC_FC:
		MANGO_DBG("SND_SOC_DAIFMT_CBM_CFM codec is clock and frame master\n");
	...
	case SND_SOC_DAIFMT_NB_NF:
		MANGO_DBG("SND_SOC_DAIFMT_NB_NF normal bit clock + frame\n");
		mcasp_set_bits(mcasp, DAVINCI_MCASP_ACLKXCTL_REG, ACLKXPOL);
		mcasp_set_bits(mcasp, DAVINCI_MCASP_ACLKRCTL_REG, ACLKRPOL);
		fs_pol_rising = true;
		break;
				      ...
				      int div, bool explicit)
{
	...
	case MCASP_CLKDIV_AUXCLK:			/* MCLK divider */
		printk("MCASP_CLKDIV_AUXCLK div_id=%d, div=%d\n",div_id,div);
	...
	case MCASP_CLKDIV_BCLK:			/* BCLK divider */
                printk("MCASP_CLKDIV_BCLK div_id=%d,div=%d\n",div_id,div);
	...
	case MCASP_CLKDIV_BCLK_FS_RATIO:
		 ...
		 */
                MANGO_DBG("MCASP_CLKDIV_BCLK_FS_RATIO div_id=%d,div=%d\n",div_id,div);
				    ...
				    unsigned int freq, int dir)
{
	...
	pm_runtime_get_sync(mcasp->dev);
        MANGO_DBG("dir=%d,clk_id=%d\n",dir,clk_id);
	if (dir == SND_SOC_CLOCK_IN) {
#if 1//crazyboys 20180826
         	switch(clk_id) {
		...
		case MCASP_CLK_HCLK_AUXCLK:
			...
			set_bit(PIN_BIT_AHCLKX, &mcasp->pdir);
			break;
		default:	// crazyboy didn't implement	
			MANGO_DBG_DEFAULT;
		}
#else
	...
	if (set) {
		if (error_ppm)
			dev_info(mcasp->dev, "Sample-rate is off by %d PPM\n",
				 error_ppm);
	//crazyboys 20180829 hamonics fix
                 div=div -1;
...
/* crazyboys 20230724 audio_refclk0=main_pll1_hsdiv6_clkout, aux_clk=main_pll1_hsdiv6_clkout */
    #define CTRL_AUDIO_REFCLK0_CTRL 0x1082e0
    #define CTRL_MCASP0_CLKSEL 0x108330

	{
		void __iomem *io = ioremap(CTRL_AUDIO_REFCLK0_CTRL, 4096);
    	/* AUDIO_REFCLK0_CTRL_CLKOUT_EN:1-output, AUDIO_REFCLK0_CTRL_CLK_SEL:6-MAIN_PLL1_HSDIV6_CLKOUT */
    	writel(0x8006, io); 
    	ret = readl(io);
    	MANGO_DBG("clk_sel=%x\n", ret);
    	iounmap(io);

    	io = ioremap(CTRL_MCASP0_CLKSEL, 4096);
    	/* MCASP0_CLKSEL_AUXCLK_SEL:1-MAIN_PLL1_HSDIV6_CLKOUT */
    	writel(0x1, io); 
    	ret = readl(io);
    	MANGO_DBG("aux_clk=%d\n", ret);
    	iounmap(io);
